...
const BlogHighLight = ({ dataPost, highlight = true }) => {
  ...
  return (
    <Link href={"/blog/" + dataPost.attributes.slug} passHref>
      <a
        className={style.lastPostSection}
        style={!highlight ? { boxShadow: "none" } : {}}
      >
        <div className={style.blogItemLastImage}>
          <img
            src={dataPost.attributes.thumbnailUrl}
            alt="mockphoto"
            width="400px"
            height="300px"
          />
        ...
        <div className={style.blogItemLastText}>
          ...
          {dataPost.attributes.content.length > 20 ? (
            <div>
              {dataPost.attributes.content.slice(
                0,
                dataPost.attributes.content.slice(0, 120).lastIndexOf(" ")
              ) + "..."}
            </div>
          ) : (
            <div>{dataPost.attributes.content}</div>
          )}
...
const BlogItem = ({ dataPost }) => {
  return (
    <Link href={"/blog/" + dataPost.attributes.slug} passHref>
      <motion.a whileHover={{ scale: 1.02 }} className={style.blogItem}>
        ...
        <div className={style.blogItemText}>
          ...
          {dataPost.attributes.content.length > 100 ? (
            <div>
              {dataPost.attributes.content.slice(
                0,
                dataPost.attributes.content.slice(0, 130).lastIndexOf(" ")
              ) + "..."}
            </div>
          ) : (
            <div>{dataPost.attributes.content}</div>
